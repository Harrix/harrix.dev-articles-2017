...
Tomcat нужен для работы Java сервера с применением [сервлетов](<https://ru.wikipedia.org/wiki/Сервлет_(Java)>). Если грубо говоря, то сервелеты это аналог тех же PHP скриптов. На сервер Tomcat от клиентов приходят запросы. В зависимости от них сервер запустит те или иные сервелеты, которые сформируют ответы в виде текстовых файлов. Чаще всего это HTML страницы.
...
Перейдем опять в папку Tomcat. Там есть папка `webapps` с web-приложениями нашего сервера. В папке `ROOT`, как не сложно догадаться, располагается корневое приложение, которое вы видели при переходе на <http://localhost:8080/>:
